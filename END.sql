...
SELECT ГРУППЫ_2011.ГРУППА
FROM
    (SELECT Н_УЧЕНИКИ.ГРУППА,
            COUNT(Н_УЧЕНИКИ.ИД) AS "КОЛ"
     FROM Н_УЧЕНИКИ
...
SELECT Н_ЛЮДИ.ИД,
       ...
       Н_ЛЮДИ.ФАМИЛИЯ,
       Н_ЛЮДИ.ОТЧЕСТВО,
       AVG(Н_ВЕДОМОСТИ.БАЛЛЫ) AS "СР_БАЛЛЫ"
FROM Н_ВЕДОМОСТИ
...
GROUP BY Н_ЛЮДИ.ИД,
         ...
         Н_ЛЮДИ.ФАМИЛИЯ,
         Н_ЛЮДИ.ОТЧЕСТВО
HAVING AVG(Н_ВЕДОМОСТИ.БАЛЛЫ) <
       (SELECT MAX(Н_ВЕДОМОСТИ.БАЛЛЫ)
        FROM Н_ВЕДОМОСТИ
                 JOIN Н_УЧЕНИКИ ON Н_УЧЕНИКИ.ЧЛВК_ИД = Н_ВЕДОМОСТИ.ЧЛВК_ИД
...
WHERE Н_УЧЕНИКИ.НАЧАЛО = '2012-09-01'
  AND Н_ПЛАНЫ.ИД IN
      (SELECT Н_НАПРАВЛЕНИЯ_СПЕЦИАЛ.ИД
...
FROM Н_ЛЮДИ
         JOIN
     (SELECT ФАМИЛИЯ,
             COUNT(ФАМИЛИЯ) AS "КОЛ"
      FROM Н_ЛЮДИ
      GROUP BY ФАМИЛИЯ) ПОВТОРЕНИЯ ON ПОВТОРЕНИЯ.ФАМИЛИЯ = Н_ЛЮДИ.ФАМИЛИЯ
         AND ПОВТОРЕНИЯ.КОЛ>1
